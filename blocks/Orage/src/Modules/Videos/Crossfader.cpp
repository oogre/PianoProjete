...
namespace ogre {
    ...
    Crossfader::Crossfader( std::string name, vec2 origin, vec2 size, gl::Context * mMainWinCtx ) : ModuleVideo(name+" "+ tools.to_roman(Crossfader::COUNT), origin, size, 2, 1, true){
    ...
    void Crossfader::update(){
        ...
        mFbo->bindFramebuffer();
        {
            ...
            if(inputs['A']){
                ...
                float a = 1.f - clamp<float>(crossfade, 0, 1);
            ...
            if(inputs['B']){
                ...
                float b = 1.f - (-1 * clamp<float>(crossfade, -1, 0));
