...
namespace ogre {
    ...
    Blur::Blur( std::string name, vec2 origin, vec2 size, gl::Context * mMainWinCtx) : ModuleVideo(name+" "+ tools.to_roman(Blur::COUNT), origin, size, 2, 1, true){
    ...
    void Blur::setupUI(){
        ...
        tools.addSlider(mUi, "X Blur", &(data.amountX), 0.f, 20);
        tools.addSlider(mUi, "Y Blur", &(data.amountY), 0.f, 20);
