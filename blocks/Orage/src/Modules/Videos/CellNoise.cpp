...
namespace ogre {
    ...
    CellNoise::CellNoise( std::string name, vec2 origin, vec2 size, gl::Context * mMainWinCtx ) : ModuleVideo(name+" "+ tools.to_roman(CellNoise::COUNT), origin, size, 1, 1, true){
    ...
    void CellNoise::setupUI(){
        ...
        tools.addSlider(mUi, "movex", &(data.movex), .0f, 1.0f);
        tools.addSlider(mUi, "movey", &(data.movey), .0f, 1.0f);
        tools.addSlider(mUi, "size", &(data.size), 1.f, 10.0f);
        tools.addSlider(mUi, "square", &(data.square), 0.f, 1.0f);
        tools.addSlider(mUi, "power", &(data.power), 0.f, 3.0f);
