...
namespace ogre {
    ...
    const int           ModuleVideo::FBO_WIDTH = 1280;//;
const int           ModuleVideo::FBO_HEIGHT = 544;
...
void ModuleVideo::setupInput(){
    ...
    for( ; it != end ; it ++, bt++){
        ...
        auto b = (*bt);
        b->setCallback(
                       [b, _w, tex, moduleName](bool a) {
                           if(a){
                               _w->clickOnVideoLinker(moduleName, tex, b, true);
                           }
                       });
    }
...
void ModuleVideo::setupOutput(){
    ...
    for( ; it != end ; it ++, bt++){
        ...
        b->setCallback(
                       [b, _w, tex, moduleName](bool a) {
                           if(a){
                               _w->clickOnVideoLinker(moduleName, tex, b, false);
                           }
                       });
    }
}
